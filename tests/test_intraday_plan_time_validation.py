...
def test_replay_disables_plan_when_vwap_confirm_seconds_is_negative(tmp_path: Path) -> None:
...
def test_replay_disables_plan_when_vwap_active_after_is_null(tmp_path: Path) -> None:
    db_path = tmp_path / "trading_x.db"
    input_path = tmp_path / "replay.csv"
    report_dir = tmp_path / "reports"
    init_db(db_path)
    ...
    with sqlite3.connect(db_path) as conn:
        conn.execute("UPDATE intraday_plans SET vwap_active_after = NULL WHERE trade_date = ?", ("20260630",))
    write_replay_csv(input_path, "20260630,09:36:00,300001.SZ,12.00,10000,1000,12.00,11.80\n")
